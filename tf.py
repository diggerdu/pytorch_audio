...
def _get_istft_kernels(n_fft, window):
    ...
    kernels = np.zeros((n_fft, n_fft//2+1)) * 1j
...
def _get_stft_kernels(n_fft, window):
    ...
    kernels = 1j * np.zeros((n_fft//2+1, n_fft), dtype=np.float32)
    ...
    print("kernel debug", np.mean(np.real(kernels)[251, :]))


    if window == "hanning":
        win_cof = scipy.signal.get_window("hanning", n_fft)[np.newaxis, :]
    else:
        win_cof = np.ones((1, n_fft), dtype=np.float32)


    '''
    print(kernels[100][600:610])
    print(win_cof.flatten()[600:610])
    kernels = kernels * win_cof
    print(kernels[100][600:610])
    '''

    kernels = kernels[:, np.newaxis, np.newaxis, :]

    real_kernels = Variable(torch.from_numpy(np.real(kernels)).double())
    imag_kernels = Variable(torch.from_numpy(np.imag(kernels)).double())
